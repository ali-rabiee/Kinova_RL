...
class RLAgentNode:
    def __init__(self):
        ...
        self.frequency = rospy.get_param('~frequency', 1)  # 0.5 Hz for 2 seconds interval
        self.automove = None
        ...
        self.stacked_keyboard_state = collections.deque(self.stack_size * [0], maxlen=self.stack_size)
    ...
    def keyboard_state_callback(self, msg):
        ...
        keyboard_state = torch.tensor([keyboard_state], dtype=torch.float32, device=self.device).unsqueeze(0).unsqueeze(0)
        self.stacked_keyboard_state.append(keyboard_state)
    ...
    def publish_action(self, event):
        ...
        try:
            ...
            if len(self.stacked_keyboard_state) < self.stack_size:
                rospy.loginfo("Not enough keyboard_state data yet")
                return
            
            stacked_frames_t = torch.cat(tuple(self.stacked_frames), dim=1).to(self.device)
            stacked_keyboard_state_t = torch.cat(tuple(self.stacked_keyboard_state), dim=1).to(self.device)
            keyboard_state_ = int(stacked_keyboard_state_t[-1, -1, -1].item())

            # Select action using the policy network
            action = self.policy_net(stacked_frames_t, stacked_keyboard_state_t).max(1)[1].view(1, 1).item()

            if keyboard_state_ == -1:
                # action = random.choices([1, 0], weights=[0.7, 0.3], k=1)[0]
                # action = random.choices([1, 0], weights=[0.9, 0.2], k=1)[0]
                action = 1

            elif keyboard_state_ == 1:
                # action = random.choices([0, 1], weights=[0.7, 0.3], k=1)[0]
                # action = random.choices([0, 1], weights=[0.9, 0.2], k=1)[0]
                action = 0

            elif keyboard_state_== 0:
                action = 3

            # elif keyboard_state_ == 2:
            #     action = 2
